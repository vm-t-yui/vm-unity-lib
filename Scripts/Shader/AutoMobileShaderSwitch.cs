...
namespace VMUnityLib
{
    public class AutoMobileShaderSwitch : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            Logger.Log(renderers.Length + " renderers");
                ...
                foreach (var r in renderers)
                {
                    ...
                    for (int n = 0; n < r.sharedMaterials.Length; ++n)
                    {
                        ...
                        if (material.shader == replacementDef.original)
                        {
                            ...
                            Logger.Log("replacing " + r.gameObject.name + " renderer " + n + " with " + newMaterials[oldMaterials.IndexOf(material)].name);
            ...
            Logger.Log(materialInstancesReplaced + " material instances replaced");
            Logger.Log(materialsReplaced + " materials replaced");
            for (int n = 0; n < oldMaterials.Count; ++n)
            {
                Logger.Log(oldMaterials[n].name + " (" + oldMaterials[n].shader.name + ")" + " replaced with " + newMaterials[n].name + " (" + newMaterials[n].shader.name + ")");
            }
#endif
        }
